...
use ratatui::{Frame, layout::{Alignment, Constraint, Direction, Layout, Rect}, style::{Color, Modifier, Style}, text::{Line, Span}, widgets::{Block, BorderType, Borders, List, ListItem, Padding, Paragraph, Wrap}};
use tungstenite::{Message, connect, handshake::client::{Request, generate_key}, protocol::{CloseFrame, frame::coding::CloseCode}};
//UTILS
fn evaluate(expr: &str) -> Result<f64,String>{
    let request = Request::builder()
            ...
            .body(())
            .map_err(|e| format!("Error al construir request {}", e))?;
    
    let (mut socket, response) = connect(request)
        .map_err(|e| format!("Conexión fallida({}): {}", e, e))?;
    if response.status() != 101 {
        return Err(format!("Servidor rechazo la peticion: {}", response.status()));
    }
    socket.send(Message::Text(expr.to_string()))
    ...
    match msg {
        ...
        Message::Close(_) => Err("El servidor cerro la conexion".into()),
...
impl App {
    pub fn new() -> Self {
        Self { input: String::new(), cursor_pos: 0, history: Vec::new(), tick: 0, show_help: false }
    }
    ...
    pub fn delete_char_before(&mut self) {
        ...
        while new_pos > 0 && !self.input.is_char_boundary(new_pos) { new_pos -= 1; }
    ...
    pub fn delete_char_after(&mut self) {
        ...
        while end < self.input.len() && !self.input.is_char_boundary(end) { end += 1; }
    ...
    pub fn clear_input(&mut self) { self.input.clear(); self.cursor_pos = 0; }
}
...
fn render_body(f: &mut Frame, area: Rect, app:&App) {
    ...
    let lines: Vec<Line> = calculator_ascii().iter().map(|row| {
        let spans: Vec<Span> = row.chars().map(|c| {
            let style = match c {
                '▓' => Style::default().fg(Color::Rgb(0, 200, 160)),
                '░' => Style::default().fg(Color::Rgb(30, 70, 60)),
                '╔'|'╚'|'╝'|'╗'|'║' => Style::default().fg(Color::Rgb(0, 200, 160)),
                '┌'|'└'|'┘'|'┐'|'│'|'─' => Style::default().fg(Color::Rgb(160, 210, 190)),
                '['|']' => Style::default().fg(Color::Rgb(0, 220, 150)).add_modifier(Modifier::BOLD),
    ...
    let panel = Paragraph::new(lines)
        .block(Block::default()
            ...
            .title(Line::from(Span::styled(" ◈ ", Style::default().fg(Color::Rgb(0, 220, 180)))))
...
fn render_prompt(f: &mut Frame, area: Rect, app:&App) {
    ...
    let (cursor_char, after_cursor) = if app.cursor_pos >= app.input.len() {
        ...
        (app.input[app.cursor_pos..end].to_string(), app.input[end..].to_string())
    };
 
    let prompt_line = Line::from(vec![
        Span::styled("  λ ", Style::default().fg(Color::Rgb(0, 220, 160)).add_modifier(Modifier::BOLD)),
        Span::styled(before.to_string(), Style::default().fg(Color::Rgb(220, 235, 225))),
        Span::styled(cursor_char, Style::default().fg(Color::Rgb(10, 14, 20)).bg(Color::Rgb(0, 220, 160))),
        Span::styled(after_cursor, Style::default().fg(Color::Rgb(220, 235, 225))),
    ]);
 
    let hint_line = Line::from(vec![
        Span::styled("  Enter", Style::default().fg(Color::Rgb(0, 200, 140)).add_modifier(Modifier::BOLD)),
        Span::styled(" calcular  ", Style::default().fg(Color::Rgb(50, 70, 65))),
        Span::styled("Esc", Style::default().fg(Color::Rgb(0, 200, 140)).add_modifier(Modifier::BOLD)),
        Span::styled(" limpiar  ", Style::default().fg(Color::Rgb(50, 70, 65))),
        Span::styled("?", Style::default().fg(Color::Rgb(0, 200, 140)).add_modifier(Modifier::BOLD)),
        Span::styled(" ayuda  ", Style::default().fg(Color::Rgb(50, 70, 65))),
        Span::styled("Ctrl+C", Style::default().fg(Color::Rgb(0, 200, 140)).add_modifier(Modifier::BOLD)),
    ...
    let widget = Paragraph::new(vec![prompt_line, hint_line])
        .block(Block::default()
            ...
            .title(Line::from(Span::styled("", Style::default().fg(Color::Rgb(150, 205, 185)))))
...
fn render_history(f: &mut Frame, area: Rect, app:&App) {
    ...
    if  app.history.is_empty() {
        ...
        items.push(ListItem::new(Line::from(vec![
            Span::styled("  ░  Sin historial.  ", Style::default().fg(Color::Rgb(45, 60, 55))),
            Span::styled("Escribe una expresión y pulsa Enter.", Style::default().fg(Color::Rgb(65, 85, 75))),
        ])));
        items.push(ListItem::new(Line::from("")));
        items.push(ListItem::new(Line::from(vec![
            Span::styled("  Ejemplos: ", Style::default().fg(Color::Rgb(55, 75, 65))),
            Span::styled("2+2  •  (3*4)/2  •  2^8  •  3.1415*2", 
                Style::default().fg(Color::Rgb(0, 170, 120)).add_modifier(Modifier::ITALIC)),
        ])));
    }else {
        for (i, entry) in app.history.iter().enumerate() {
            ...
            let expr_line = Line::from(vec![
                ...
                Span::styled(entry.expr.clone(), Style::default().fg(Color::Rgb(160, 200, 185))),
            ]);
 
            let result_line = match &entry.result {
                Ok(v) => Line::from(vec![
                    Span::styled("    │   = ", Style::default().fg(Color::Rgb(45, 60, 55))),
                    Span::styled(fmt_result(*v), Style::default().fg(Color::Rgb(0, 230, 160)).add_modifier(Modifier::BOLD)),
                ]),
                Err(e) => Line::from(vec![
            ...
            let sep_line = Line::from(Span::styled(sep, Style::default().fg(Color::Rgb(28, 38, 50))));
    ...
    let list = List::new(items)
        .block(Block::default()
            ...
            .title(Line::from(vec![
                ...
                Span::styled(format!("  {} op. ", app.history.len()), Style::default().fg(Color::Rgb(45, 65, 85))),
            ]))
...
fn render_help(f: &mut Frame,area: Rect) {
    ...
    let lines: Vec<Line> = vec![
        Line::from(""),
        Line::from(vec![Span::styled("  Operadores", Style::default().fg(Color::Rgb(0, 210, 160)).add_modifier(Modifier::BOLD))]),
        Line::from(vec![Span::styled("  +  -  *  /  ^  %", Style::default().fg(Color::Rgb(180, 220, 200)))]),
        Line::from(""),
        Line::from(vec![Span::styled("  Ejemplos", Style::default().fg(Color::Rgb(0, 210, 160)).add_modifier(Modifier::BOLD))]),
        Line::from(vec![Span::styled("  (2+3)*4   2^8", Style::default().fg(Color::Rgb(180, 220, 200)))]),
        Line::from(vec![Span::styled("   3.1415*r^2", Style::default().fg(Color::Rgb(180, 220, 200)))]),
        Line::from(""),
        Line::from(vec![
            Span::styled("  Pulsa ", Style::default().fg(Color::Rgb(75, 100, 90))),
            Span::styled("?", Style::default().fg(Color::Rgb(0, 220, 160)).add_modifier(Modifier::BOLD)),
            Span::styled(" o ", Style::default().fg(Color::Rgb(75, 100, 90))),
            Span::styled("Esc", Style::default().fg(Color::Rgb(0, 220, 160)).add_modifier(Modifier::BOLD)),
    ...
    let help = Paragraph::new(lines)
        .block(Block::default()
            ...
            .title(Line::from(vec![
                ...
                Span::styled("Ayuda", Style::default().fg(Color::White).add_modifier(Modifier::BOLD)),
